...
async function run(){
    try{
        const appointOptionsCollection = client.db('doctorsPortal').collection('appointmentOptions');
        ...
        app.get('/appointOptions', async(req, res) => {
            ...
            console.log(date);
            const query = {}
            const cursor = appointOptionsCollection.find(query);
        ...
        app.get('/appointmentSpecialty', async (req, res) => {
            const query = {}
            const result = await appointOptionsCollection.find(query).project({ name: 1 }).toArray();
            res.send(result);
        })
        ...
        //         }
        //     }
        //     const result = await appointOptionsCollection.updateMany(filter, updatedDoc, options);
        //     res.send(result);
        // })
